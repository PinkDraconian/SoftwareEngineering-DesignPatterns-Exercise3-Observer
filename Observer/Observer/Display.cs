...
namespace Observer
{
    ...
    class Piano : IDisplay, IDisposable
    {
        private WeatherData weatherData = null;
        private WeatherMeasurements readings = new WeatherMeasurements();
        ...
        {
            string pianoString;
            if (this.readings.humidity < 45)
            {
                pianoString = "Too Dry in here!";
            }
            else if (this.readings.humidity > 55)
            {
                pianoString = "Too Wet in here!";
            }
            else
            {
                pianoString = "Music Maestro";
            }
            Console.WriteLine("Piano: {0}", pianoString);
        }
